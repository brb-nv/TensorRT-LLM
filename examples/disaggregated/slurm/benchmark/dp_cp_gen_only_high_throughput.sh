...
# =============================================================================
# PURE CP COMBINATIONS - TP=1, CP=num_gpus (TEP mode, AttnDP=false; AttnDP is a no-op with TP=1)
# These are separated as they may require special handling (e.g., CUDA graph min_batch_size)
# =============================================================================
PURE_CP_COMBINATIONS=(
    # tep_4: TP=1, CP=4 (AttnDP=false; no-op with TP=1)
    "4,8,131072,8192,1,1,4,4,0"
    "4,16,131072,8192,1,1,4,4,0"
    "4,32,131072,8192,1,1,4,4,0"
    "4,64,131072,8192,1,1,4,4,0"
    "4,128,131072,8192,1,1,4,4,0"
    # tep_8: TP=1, CP=8 (AttnDP=false; no-op with TP=1)
    "8,16,131072,8192,1,1,8,8,0"
    "8,32,131072,8192,1,1,8,8,0"
    "8,64,131072,8192,1,1,8,8,0"
    "8,128,131072,8192,1,1,8,8,0"
    "8,256,131072,8192,1,1,8,8,0"
    "8,512,131072,8192,1,1,8,8,0"
    # tep_16: TP=1, CP=16 (AttnDP=false; no-op with TP=1)
    "16,32,131072,8192,1,1,16,16,0"
    "16,64,131072,8192,1,1,16,16,0"
    "16,128,131072,8192,1,1,16,16,0"
    "16,256,131072,8192,1,1,16,16,0"
    "16,512,131072,8192,1,1,16,16,0"
    "16,1024,131072,8192,1,1,16,16,0"
    # tep_32: TP=1, CP=32 (AttnDP=false; no-op with TP=1)
    "32,32,131072,8192,1,1,32,32,0"
    "32,64,131072,8192,1,1,32,32,0"
    "32,128,131072,8192,1,1,32,32,0"
    "32,256,131072,8192,1,1,32,32,0"
    "32,512,131072,8192,1,1,32,32,0"
    "32,1024,131072,8192,1,1,32,32,0"
)
...
usage() {
    echo "Usage: $0 [gen2|gen4|gen8|gen16|gen32|pure_cp|all|all_with_pure_cp]"
    echo ""
    ...
    echo "  pure_cp          Run pure CP combinations only (TP=1, CP=num_gpus)"
    echo "  all              Run all combinations where CP <= TP (default)"
    echo "  all_with_pure_cp Run all combinations including pure CP cases"
    echo ""
    echo "Naming convention:"
    echo "  dep_N  - DEP mode with N GPUs (AttnDP=true)"
    echo ""
    echo "Note: Pure CP cases are separated because they require min_batch_size >= CP"
...
case "$MODE" in
    ...
    gen32)
        ...
        ;;
    pure_cp)
        COMBINATIONS=("${PURE_CP_COMBINATIONS[@]}")
        MODE_DESC="Pure CP combinations"
        ;;
    all)
        ...
        ;;
    all_with_pure_cp)
        COMBINATIONS=("${GEN2_COMBINATIONS[@]}" "${GEN4_COMBINATIONS[@]}" "${GEN8_COMBINATIONS[@]}" "${GEN16_COMBINATIONS[@]}" "${GEN32_COMBINATIONS[@]}" "${PURE_CP_COMBINATIONS[@]}")
        MODE_DESC="All DP/CP High Throughput (including pure CP)"
        ;;
    -h|--help)
        usage
        ;;
...
if [[ "$MODE" == "pure_cp" || "$MODE" == "all_with_pure_cp" ]]; then
    echo "Note: Pure CP cases included - CUDA graph min_batch_size will be set to CP"
fi
echo "============================================"
