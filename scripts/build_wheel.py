...
def apply_msa_patch(project_dir: Path) -> None:
    ...
    The patch is applied directly to the pinned ``3rdparty/MSA`` submodule
    working tree so every consumer sees the patched ``fmha_sm100`` sources: the
    runtime importer that prepends ``3rdparty/MSA/python`` to ``sys.path``,
    editable installs (``pip install -e .``), and wheel packaging. Re-running is
    a no-op -- if the patch already reverse-applies cleanly it is in place and
    the function returns early -- so it is safe to call on every build,
    including ``--skip_building_wheel`` (-s) runs.
    """
    ...
    # Idempotency guard: a clean reverse-apply means the patch is already in the
    # working tree, so a forward apply would fail. Skip in that case.
    already_applied = run(
...
def main(*,
    ...
    # Apply the downstream MSA patch in place before any packaging so that
    # -s / editable / wheel builds and the runtime importer all see it.
