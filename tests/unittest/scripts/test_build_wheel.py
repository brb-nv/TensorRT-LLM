...
def _stage_project(tmp_path: Path) -> Path:
    ...
    The real submodule tree is left untouched; the copy drops ``.git`` so the
    apply runs against a plain working tree (as it does after a fresh checkout).
    """
    ...
    shutil.copytree(source_msa,
                    project_dir / "3rdparty" / "MSA",
...
def test_apply_msa_patch_is_idempotent_in_place(tmp_path):
    ...
    # A second call must short-circuit via the reverse-check guard, not raise,
    # and must leave the patched content in place.
    apply_msa_patch(project_dir)
    assert "def _prepare_paged_hnd_input" in patched_interface.read_text()
