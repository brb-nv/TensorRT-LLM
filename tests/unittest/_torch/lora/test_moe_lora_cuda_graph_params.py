...
that previously-untested glue end-to-end on the Python side, without model
weights or the built C++ op:
...
def test_get_moe_slot_inputs_packs_pointers_and_aliases_ranks():
    ...
    assert params.get_moe_slot_inputs(layer_idx=0, module_id=int(LoraModuleType.ATTENTION_QKV)) is None
    assert params.get_moe_slot_inputs(layer_idx=99, module_id=int(LoraModuleType.MOE_H_TO_4H)) is None
...
def test_get_moe_slot_inputs_packed_buffer_is_address_stable():
    ...
    data_ptr() is stable across calls -- required for CUDA-graph capture."""
    rank = 4
    params, key, module_ids = _make_params(max_lora_size=2, max_rank=rank)
    _populate_pointers(params, key, module_ids, rank)
...
def test_extract_returns_none_without_layer_or_required_modules():
    ...
    assert CutlassFusedMoE._extract_moe_lora_tensors_cuda_graph(
        _ExtractStub(0), {"num_seqs": 1}) is None
    ...
    fc1_only = CudaGraphLoraParams(
        ...
        layer_info={
            fc1_only_key:
            CudaGraphLoraParams.LoraLayerInfo(module_num=1, output_sizes=[_INTER])
        },
    )
    assert CutlassFusedMoE._extract_moe_lora_tensors_cuda_graph(
        _ExtractStub(0), {"cuda_graph_params": fc1_only, "num_seqs": 1}) is None
