...
def test_check_moe_lora_cutlass_fp8_block_scale_ok():
    # FP8 block-scale base weights are supported on Cutlass.
    cfg = _FakeLoraConfig(["moe_h_to_4h", "moe_4h_to_h", "moe_gate"])
    check_moe_lora_supported(
        moe_backend_name="CUTLASS",
        lora_config=cfg,
...
def test_check_moe_lora_rejects_nvfp4_base():
    # FP4 (and other non-FP8 quant) has no LoRA path and must be rejected.
    cfg = _FakeLoraConfig(["moe_gate", "moe_4h_to_h"])
    with pytest.raises(ValueError, match="FP4/INT4/INT8/W4A8"):
        check_moe_lora_supported(
            moe_backend_name="CUTLASS",
            lora_config=cfg,
