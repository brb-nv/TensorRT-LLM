...
requires_cuda_and_op = pytest.mark.skipif(
    ...
    reason=
    "Requires CUDA and built TensorRT-LLM C++ extension (torch.ops.trtllm.fused_moe).",
)
...
def _build_base_inputs(num_tokens, hidden_size, inter_size, num_experts, top_k,
    ...
    w3_w1 = torch.randn(
        num_experts, 2 * inter_size, hidden_size, dtype=dtype,
        device=device) * 0.02
    w2 = torch.randn(
        num_experts, hidden_size, inter_size, dtype=dtype, device=device) * 0.02
...
def _make_adapter_set(num_experts, rank, hidden_size, inter_size, dtype, device,
                      base_seed):
    ...
    fc1 = _scaled(num_experts, rank, hidden_size, inter_size,
                  seed=base_seed + 0)
    gated = _scaled(num_experts, rank, hidden_size, inter_size,
                    seed=base_seed + 1)
    fc2 = _scaled(num_experts, rank, inter_size, hidden_size,
                  seed=base_seed + 2)
...
def _slot_kwargs(token_to_slot, adapter_sets, rank):
    ...
    def _ptrs(key):
        return torch.tensor(
            [[a[key]["A"].data_ptr(), a[key]["B"].data_ptr(), 0]
             for a in adapter_sets],
            dtype=torch.int64, device="cpu").pin_memory()

    ranks = torch.full((max_lora_size, ), rank, dtype=torch.int32,
...
def _reference(x, w3_w1, w2, topk_ids, topk_scores, adapters):
    return reference_swiglu_moe_lora(
        ...
        fc1_a=adapters["fc1"]["A"], fc1_b=adapters["fc1"]["B"],
        gated_a=adapters["gated"]["A"], gated_b=adapters["gated"]["B"],
        fc2_a=adapters["fc2"]["A"], fc2_b=adapters["fc2"]["B"])


def _reference_multi_slot(x, w3_w1, w2, topk_ids, topk_scores, adapter_sets,
                          token_to_slot):
    ...
    """
    per_slot = [
        _reference(x, w3_w1, w2, topk_ids, topk_scores, a) for a in adapter_sets
    ]
...
def test_device_path_eager_matches_host_and_reference(monkeypatch):
    ...
    try:
        out_host = _call_fused_moe(x, w3_w1, w2, topk_ids, topk_scores, dtype,
                                   dict(lora_kwargs))
    finally:
        MoERunner.runner_dict.clear()
    ...
    MoERunner.runner_dict.clear()
    try:
        out_device = _call_fused_moe(x, w3_w1, w2, topk_ids, topk_scores, dtype,
                                     dict(lora_kwargs))
    finally:
        MoERunner.runner_dict.clear()
...
def test_device_path_cuda_graph_multi_adapter():
    ...
    x, w3_w1, w2, topk_ids, topk_scores = _build_base_inputs(
        num_tokens, hidden_size, inter_size, num_experts, top_k, dtype, device)
    adapter_a = _make_adapter_set(num_experts, rank, hidden_size, inter_size,
                                  dtype, device, base_seed=500)
    adapter_b = _make_adapter_set(num_experts, rank, hidden_size, inter_size,
                                  dtype, device, base_seed=600)
    ...
    out_ref = _reference_multi_slot(x, w3_w1, w2, topk_ids, topk_scores,
                                    [adapter_a, adapter_b], token_to_slot)
    assert torch.isfinite(out_replay).all()
    torch.testing.assert_close(out_replay, out_ref, rtol=_RTOL, atol=_ATOL)
...
def test_device_path_slot_reassignment_reflected_on_replay():
    ...
    x, w3_w1, w2, topk_ids, topk_scores = _build_base_inputs(
        num_tokens, hidden_size, inter_size, num_experts, top_k, dtype, device)
    adapter_a = _make_adapter_set(num_experts, rank, hidden_size, inter_size,
                                  dtype, device, base_seed=700)
    adapter_b = _make_adapter_set(num_experts, rank, hidden_size, inter_size,
                                  dtype, device, base_seed=800)
    ...
    assert (out_ref_a.float() -
            out_ref_b.float()).abs().mean().item() > 1e-2
