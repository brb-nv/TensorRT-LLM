...
class TestGemma3(unittest.TestCase):
    ...
    def test_gemma3_flashinfer_mask(self):
        ...
        # Causal mask for generation request 1.
        gen_request_1_mask = torch.tensor([[True, True, True]], device=device)
        # Causal mask for generation request 1.
        gen_request_2_mask = torch.tensor([[True, True]], device=device)

        expected_causal_mask = torch.cat([
            ctx_request_1_mask.flatten(),
            ctx_request_2_mask.flatten(),
            ctx_request_3_mask.flatten(),
            gen_request_1_mask.flatten(),
            gen_request_2_mask.flatten(),
        ],
                                         dim=0)
    ...
    def test_gemma3_global_generation_mask(self) -> None:
        config_dict = deepcopy(GEMMA3_1B_MINI_CONFIG)
        gemma3_config = Gemma3Config.from_dict(config_dict)
        device = torch.device('cuda')
        model_config = ModelConfig(pretrained_config=gemma3_config,
        ...
        generation_mask = gemma3.get_generation_mask(
            cached_token_len=5, device=device, effective_sliding_window=None)
        print("[TestGemma3::test_gemma3_generation_mask] generation_mask: \n",
              generation_mask)
        expected_generation_mask = torch.tensor(
            [True, True, True, True, True, True], device=device)
        torch.testing.assert_close(generation_mask, expected_generation_mask)

        generation_mask = gemma3.get_generation_mask(cached_token_len=5,
                                                     device=device,
                                                     effective_sliding_window=6)
        print("[TestGemma3::test_gemma3_generation_mask] generation_mask: \n",
              generation_mask)
        torch.testing.assert_close(generation_mask, expected_generation_mask)

    def test_gemma3_local_generation_mask(self) -> None:
        config_dict = deepcopy(GEMMA3_1B_MINI_CONFIG)
        gemma3_config = Gemma3Config.from_dict(config_dict)
        device = torch.device('cuda')
        model_config = ModelConfig(pretrained_config=gemma3_config,
        ...
        generation_mask = gemma3.get_generation_mask(cached_token_len=5,
                                                     device=device,
                                                     effective_sliding_window=5)
        print("[TestGemma3::test_gemma3_generation_mask] generation_mask: \n",
              generation_mask)
        expected_generation_mask = torch.tensor(
            [False, True, True, True, True, True], device=device)
        torch.testing.assert_close(generation_mask, expected_generation_mask)
