...
class TestGemma3VisionModel(unittest.TestCase):
    ...
    @torch.no_grad()
    def test_gemma3_allclose_to_hf(self) -> None:
        """
        Compare output of TRTLLM vision tower to HF.
        """
        get_attention_backend("TRTLLM").Metadata
        ...
        hf_vision_tower = module_dict["vision_tower"].to(dtype).to(
            device).eval()

        vision_model_config = ModelConfig(
            pretrained_config=hf_model_config.vision_config,
            attn_backend="TRTLLM")
        trtllm_vision_tower = SiglipVisionModel(
            vision_model_config, use_post_layernorm=True).to(dtype).to(device)
        trtllm_vision_tower.load_weights(hf_vision_tower.state_dict())

        pixel_values = torch.rand(1,
                                  3,
                                  896,
                                  896,
        ...
        with torch.inference_mode():
            attn_metadata = trtllm_vision_tower.prepare_attn_metadata(
                pixel_values.shape[0])
            trtllm_image_features = trtllm_vision_tower(
                pixel_values, attn_metadata=attn_metadata)
            hf_image_features = hf_vision_tower(pixel_values)

            print(
                f"[test_gemma3_allclose_to_hf] trtllm_image_features: {trtllm_image_features[-1].shape} \n trtllm_image_features: {trtllm_image_features[-1]}"
            )
            print(
                f"[test_gemma3_allclose_to_hf] hf_image_features: {hf_image_features.last_hidden_state.shape} \n hf_image_features: {hf_image_features.last_hidden_state}"
            )
            assert trtllm_image_features[
                -1].shape == hf_image_features.last_hidden_state.shape
            print("[test_gemma3_allclose_to_hf] max diff:",
                  (trtllm_image_features[-1] -
                   hf_image_features.last_hidden_state).abs().max())
            print("[test_gemma3_allclose_to_hf] mean diff:",
                  (trtllm_image_features[-1] -
                   hf_image_features.last_hidden_state).abs().mean())
