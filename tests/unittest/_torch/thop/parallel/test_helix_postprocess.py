...
def baseline(gathered_o, gathered_stats, kv_lora_rank, scale, native=False, native_v2=False):
    """Reference implementation (libtorch)

    Args:
        gathered_o: Input tensor
            - native=False: [cp_size, num_tokens, num_heads * kv_lora_rank]
            - native=True (cp_dim=2): [num_tokens, num_heads, cp_size, kv_lora_rank]
        ...
        gathered_stats: Stats tensor
            ...
            - native=True (cp_dim=2): [num_tokens, num_heads, cp_size, 2]
...
class TestHelixPostProcess(unittest.TestCase):
    ...
    def _test_helix_postprocess_v2(
        self, cp_size, num_tokens, num_heads, kv_lora_rank, scale, dtype
    ):
        """Test helix postprocessing with cp_dim=1 (Version 2) layout
        ...
        # Call the custom operator with cp_dim=1
        output = torch.ops.trtllm.helix_post_process_native(
            gathered_o, gathered_stats, scale, 1
        )

        # Compute baseline
        expected_output = baseline(gathered_o, gathered_stats, kv_lora_rank, scale, native_v2=True)
    ...
    def test_helix_postprocess_v2_basic(
        self, cp_size, num_tokens, num_heads, kv_lora_rank, scale, dtype
    ):
        """Test basic helix postprocessing with cp_dim=1 (Version 2)"""
        self._test_helix_postprocess_v2(
            cp_size, num_tokens, num_heads, kv_lora_rank, scale, dtype
        )
    ...
    def test_helix_postprocess_v2_edge_cases(
        self, cp_size, num_tokens, num_heads, kv_lora_rank, scale, dtype
    ):
        """Test edge cases with minimal dimensions for cp_dim=1 (Version 2)"""
        self._test_helix_postprocess_v2(
            cp_size, num_tokens, num_heads, kv_lora_rank, scale, dtype
        )
    ...
    @parameterized.expand(
        [
            # (cp_size, num_tokens, num_heads, kv_lora_rank, scale, dtype)
            (16, 16, 64, 512, 1.0, torch.float16),
            (16, 16, 64, 512, 1.0, torch.bfloat16),
        ]
    )
    def test_helix_postprocess_v2_large_inputs(
        self, cp_size, num_tokens, num_heads, kv_lora_rank, scale, dtype
    ):
        """Test with larger inputs for cp_dim=1 (Version 2)"""
        self._test_helix_postprocess_v2(
            cp_size, num_tokens, num_heads, kv_lora_rank, scale, dtype
        )

    def test_helix_postprocess_v2_alignment_requirements(self):
        """Test alignment requirements for cp_dim=1 (Version 2)"""
        device = torch.device("cuda")
