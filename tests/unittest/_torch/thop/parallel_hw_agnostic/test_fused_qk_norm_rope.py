...
def test_fused_qk_norm_rope_to_fp8_meta_keeps_dynamic_num_tokens():
    """The meta kernel must propagate a symbolic token dimension.
    ...
    qkv = torch.empty(8, hidden_size, dtype=torch.bfloat16, device="cuda")
    position_ids = torch.arange(8, dtype=torch.int32, device="cuda")
    q_weight = torch.empty(head_dim, dtype=torch.bfloat16, device="cuda")
    k_weight = torch.empty(head_dim, dtype=torch.bfloat16, device="cuda")
    ...
    assert isinstance(out.shape[0], torch.SymInt), (
        f"num_tokens was specialized to {out.shape[0]}; the meta kernel must use sym_size"
    )
