...
#    reference.
#  * Sparse index branch: the index Q and K projections are fused into a
#    single **replicated** (tp_mode is None) ``index_qk_proj`` whose output is
#    ``[idx_q | idx_k]`` = ``num_index_heads * sparse_index_dim`` (per-head Q)
#    followed by ``sparse_index_dim`` (single K per token, broadcast across all
#    index heads for block-selection scoring) — this is the SGLang reference
#    contract. The two source tensors ``index_q_proj.weight`` (512, 6144) and
#    ``index_k_proj.weight`` (128, 6144) are concatenated into
#    ``index_qk_proj.weight`` (640, 6144) at load time.
...
def test_minimax_m3_attention_sparse_construction_matches_config():
    ...
    Verifies the fused index projection:
      * ``index_qk_proj`` is replicated (``tp_mode is None``) and projects to
        ``num_index_heads * sparse_index_dim`` (idx_q) + ``sparse_index_dim``
        (idx_k) — the idx_k slice is a single replicated K per token, *not*
        per-head, matching SGLang's ``ReplicatedLinear`` and the M3
        checkpoint's ``index_k_proj.weight`` shape ``(sparse_index_dim,
        hidden_size)``.
    ...
    # index_qk_proj: the index Q and K projections are fused into one
    # replicated (tp_mode=None) GEMM whose output is [idx_q | idx_k]. It is
    # replicated, not column-parallel, because the sparse forward consumes
    # ``idx_q`` reshaped to ``[num_tokens, num_index_heads, sparse_index_dim]``
    # and a column-parallel split would slice the head dimension (breaking the
    # reshape at any ``tp_size > num_index_heads`` geometry, including the TP=8
    # configuration the real-checkpoint smoke test uses). The replicated weight
    # is small (~4 MiB BF16) so the per-rank memory cost is negligible.
    ...
    assert attn.index_qk_proj.tp_mode is None, (
        f"index_qk_proj must be replicated (tp_mode=None) so the sparse "
        f"forward's `idx_q.view(num_tokens, num_index_heads, sparse_index_dim)` "
        f"reshape is well-defined at any TP geometry, got "
        f"{attn.index_qk_proj.tp_mode!r}"
    )
    # The two projections must no longer exist as separate modules.
...
def _make_fused_qk_norm_rope_test_config():
    """``(text_config, ModelConfig)`` with a kernel-supported head_dim.

    ``fused_qk_norm_rope`` only compiles for ``head_dim in {64, 128, 256}``,
    so the scaled-down ``_make_attention_test_config`` (head_dim=32) cannot
    drive the fused path. This variant uses the real M3 ``head_dim=128`` /
    ``sparse_index_dim=128`` / ``rotary_dim=64`` geometry with a small head
    count so the tensors stay tiny.
    """
    n_layers = 4
    sparse_cfg = {
        "use_sparse_attention": True,
...
def test_minimax_m3_fused_qk_norm_rope_main_matches_separate():
    """``_fused_qk_norm_rope`` (main branch) == separate norm + partial RoPE.

    Exercises the actual layer helper end-to-end so the wiring — reading
    ``rope.dim`` for the partial rotary dim, ``rope.theta``, ``is_neox``,
    ``use_gemma_norm``, per-head norm weights, and ``variance_epsilon`` — is
    validated against the fallback ``apply_qk_norm`` + ``rotary_emb`` path
    ...
    torch.manual_seed(0)
    attn.q_norm.weight = torch.nn.Parameter(
        torch.randn(head_dim, dtype=dtype, device=device) * 0.2
    )
    attn.k_norm.weight = torch.nn.Parameter(
        torch.randn(head_dim, dtype=dtype, device=device) * 0.2
    )

    seq = 6
    qkv = torch.randn(
        seq, attn.q_size + 2 * attn.kv_size, dtype=dtype, device=device
    )
...
def test_minimax_m3_fused_qk_norm_rope_index_matches_separate():
    """``_fused_qk_norm_rope`` (index branch, num_heads_v=0) == separate path.

    The index branch concatenates ``idx_q`` (``sparse_num_index_heads``
    heads) and ``idx_k`` (1 replicated head), passes ``num_heads_v=0`` so
    the kernel norms/rotates only the Q/K segments, and splits back.
    """
    _, model_cfg = _make_fused_qk_norm_rope_test_config()
    attn = MiniMaxM3Attention(
        model_config=model_cfg,
    ...
    iq_f, ik_f = fused.split(
        [num_index_heads * sparse_index_dim, sparse_index_dim], dim=-1
    )
...
def test_minimax_m3_fused_qk_norm_rope_fallbacks():
    """The fused helper returns None (fallback) for non-bf16 / no position_ids."""
    _, model_cfg = _make_fused_qk_norm_rope_test_config()
    attn = MiniMaxM3Attention(
        model_config=model_cfg,
        layer_idx=0,
    ...
    # No position_ids -> RoPE cannot run -> fallback.
    ...
    # Non-bf16 activations (fp8/fp4 KV cache configs) -> kernel is bf16-only
    # -> fallback.
...
def test_minimax_m3_expect_fused_qk_norm_rope_predicate():
    ...
    For every M3 quant flavor with bf16 attention activations — plain
    BF16, MXFP8 (MXFP8 weights, bf16 activations), and the NVFP4
    "FP4" checkpoint (MXFP8 base + NVFP4 experts, bf16 activations) —
    the main (head_dim=128) and index (sparse_index_dim=128) branches
    must expect the fused kernel, so a runtime fallback trips the
    assertion. Non-bf16 activations (would only arise if qkv came back
    quantized) or missing position_ids relax the expectation.
    """
    _, model_cfg = _make_fused_qk_norm_rope_test_config()
    attn = MiniMaxM3Attention(
        model_config=model_cfg,
    ...
    # bf16 activations (the case for BF16 / MXFP8 / NVFP4 M3 checkpoints)
    # -> fusion is required for both branches.
    assert attn.attn_activation_dtype == torch.bfloat16
    assert attn._expect_fused_qk_norm_rope(position_ids, attn.head_dim) is True
    assert attn._expect_fused_qk_norm_rope(position_ids, attn.sparse_index_dim) is True

    # No position_ids -> RoPE cannot run -> fallback allowed.
    assert attn._expect_fused_qk_norm_rope(None, attn.head_dim) is False

    # A head_dim the kernel is not templated for -> fallback allowed.
    assert attn._expect_fused_qk_norm_rope(position_ids, 96) is False

    # Non-bf16 activations (would require an fp8/fp4 *activation* path,
    # which M3 does not use) -> fallback allowed, no assertion.
    attn.attn_activation_dtype = torch.float16
    assert attn._expect_fused_qk_norm_rope(position_ids, attn.head_dim) is False
...
def test_minimax_m3_attention_real_config_index_branch_shapes():
    ...
    Asserts the fused index projection in numbers:
      * ``index_qk_proj.out_features == 640`` (= 4 * 128 + 128 =
        ``num_index_heads * sparse_index_dim`` (idx_q) + ``sparse_index_dim``
        (idx_k)) and ``tp_mode is None`` (replicated). The checkpoint ships the
        two source tensors ``index_q_proj.weight`` (512, 6144) and
        ``index_k_proj.weight`` (128, 6144); they are concatenated into
        ``index_qk_proj.weight`` (640, 6144) at load time.
    ...
    # index_qk_proj: fused [idx_q | idx_k] = (4 * 128) + 128 = 640 out,
    # replicated (tp_mode=None). The downstream sparse forward reshapes
    # ``idx_q`` to ``[num_tokens, num_index_heads, sparse_index_dim]``; a
    # column-parallel split would slice the head dimension and break that
    # reshape at any ``tp_size > num_index_heads`` geometry (including TP=8 used
    # by the real-checkpoint smoke test). The replicated weight is ~4 MiB BF16 —
    # the per-rank memory cost is negligible. The two checkpoint tensors
    # (index_q_proj.weight / index_k_proj.weight) are concatenated into
    # index_qk_proj.weight at load time.
    assert attn.index_q_size == num_index_heads * sparse_index_dim
    assert attn.index_k_size == sparse_index_dim
