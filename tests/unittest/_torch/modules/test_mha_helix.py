...
def _run_attention_distributed(
    ...
    input_ctx_bs_rank = input_ctx_bs[:,
    ...
    for step in range(gen_steps):
        ...
        if step == 0:
            ...
            attn_metadata.helix_is_inactive_rank_cpu = (
                attn_metadata.helix_is_inactive_rank.to(
            ...
            attn_metadata.helix_position_offsets_cpu = (
                attn_metadata.helix_position_offsets.to(
                    device="cpu").pin_memory())
        else:
        ...
        if step == 0:
            print(f"Rank {rank} {world_size}-GPU: "
                  f"Creating CUDA graph and capturing")
            ...
            with torch.cuda.graph(graph):
                with model_extra_attrs(extra_attrs):
                    graph_output = attn(position_ids_gen, input_gen,
                                        attn_metadata)
            result = graph_output
    ...
    if rank == 0:
        ...
        for step in range(gen_steps):
            ...
            if step == 0:
                ...
                ref_attn_metadata = (
                    ref_attn_metadata.create_cuda_graph_metadata(
                        max_batch_size=scenario.batch))
                ref_attn_metadata.prepare()

                for _ in range(2):
                    result = attn(position_ids_gen, input_gen,
                                  ref_attn_metadata)
    ...
    for comms_medium in ["fifo_v1", "fifo_v2", "nccl"]:
        ...
        for scenario in all_scenarios[:8]:
            ...
            if any(mismatch > 0 for mismatch in mismatch_ratios):
                print(
                    f"Numerical test failed with mismatch ratios: "
                    f"{mismatch_ratios}")
            else:
