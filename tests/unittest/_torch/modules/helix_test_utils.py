...
def create_helix_spec_gen_metadata(
    ...
    *,
    ...
    Each request submits ``num_query_tokens`` query tokens. Ownership is per
    request (per verify group): ``helix_is_inactive_rank`` has length ``batch``
    (one flag per sequence), so the whole group of query tokens is owned by a
    single CP rank. The owned token count per sequence (used for kv_lens) is
    derived from those flags in ``prepare()``. The linear-chain causal mask is
    generated by update_spec_dec_param.
    """
    ...
    # position buffer, per request for the inactive-flag and total_input_len
    # buffers).
