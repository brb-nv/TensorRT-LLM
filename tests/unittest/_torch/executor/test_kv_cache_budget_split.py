...
class TestBuildManagersBudgetGates:
    ...
    def _configs_passed_to_managers(c: KvCacheCreator):
        return (
            ...
            c._create_one_model_draft_kv_cache_manager.call_args.kwargs[
    ...
    def test_no_manager_receives_the_full_host_budget(self, estimating_kv_cache):
        """Regression: target and draft each pinned the whole host tier.

        Every manager honors ``host_cache_size`` verbatim and prefaults and
        page-locks it at construction, so two managers holding the full budget
        at once made a run need twice the configured host memory.
        """
        total_host = 20 * GB
    ...
    def test_estimation_managers_fall_back_to_the_auto_host_tier(self):
        """Estimation probes size their GPU pool from max_tokens, so an explicit
        host tier is memory pinned to back a cache that can never fill."""
