...
@pytest.mark.parametrize("pp_size,enable_overlap_headroom,expected_factor",
                         DISAGG_SIZING_CASES)
def test_disagg_seats_cover_index_mapper_capacity(pp_size,
                                                  enable_overlap_headroom,
                                                  expected_factor):
    max_batch_size = 8
    mapping = Mapping(world_size=pp_size, tp_size=1, pp_size=pp_size)
...
def test_disagg_seats_do_not_depend_on_overlap_scheduler(disable_overlap):
    ...
    assert (compute_max_num_sequences(
        mapping, max_batch_size, disable_overlap,
        is_disagg=True) == max_batch_size * 2)
...
def test_aggregate_sizing_is_unchanged():
    ...
    assert (compute_max_num_sequences(mapping,
                                      max_batch_size,
                                      disable_overlap_scheduler=False,
                                      is_disagg=False) == max_batch_size)
...
def test_sizing_matches_kv_manager_admission_bound(pp_size, is_disagg):
    ...
    seats = compute_max_num_sequences(mapping,
                                      max_batch_size,
                                      disable_overlap_scheduler=False,
                                      is_disagg=is_disagg)
