...
def test_fanin_bounce_safe_gate() -> None:
    ...
    # per writer for a pure-TP fan-in; combined with a PP fan-in it still falls back. Both page
    # tables are checked there because the representative peer rank may be a fully masked PP stage
    # while another sender stage owns the replicated rows.
    ...
    assert safe(ov(1, 1, ranks=(0, 1)), ri([24], pt(MapperKind.NHD)), None) is True
    assert (
        safe(ov(1, 4, ranks=(0, 1)), ri([20, 20, 20, 20], pt(MapperKind.REPLICATED)), None) is False
    )
    assert (
        safe(
            ov(1, 4, ranks=(0, 1)),
            ri([20, 20, 20, 20], pt(MapperKind.NHD)),
