...
Covers unquantized bf16 (Qwen1.5-MoE), per-tensor FP8 qdq (Mixtral-8x7B) and
FP8 block-scale (DeepSeek-V3-Lite) base weights, with several adapters of
...
from ..conftest import get_sm_version, llm_models_root, skip_pre_ada

# The DeepSeek-style FP8 block-scale grouped GEMM is only implemented for SM90
# (Hopper) and SM120; on other architectures (e.g. SM100 datacenter Blackwell)
# the kernel throws, so block-scale MoE LoRA cannot run there.
_FP8_BLOCK_SCALE_SUPPORTED_SM = (90, 120)
skip_if_no_fp8_block_scale = pytest.mark.skipif(
    get_sm_version() not in _FP8_BLOCK_SCALE_SUPPORTED_SM,
...
def test_deepseek_moe_routed_expert_fp8_block_scale_multi_lora_varying_ranks(
    ...
    With block-scale FP8 the base GEMM quantizes activations per block
    internally and the LoRA GEMM runs on the bf16 activations directly. Forces
    the capture-safe device LoRA path (TLLM_MOE_LORA_USE_DEVICE_PATH=1).
    """
    ...
    model_dir = f"{llm_models_root()}/DeepSeek-V3-Lite/fp8"
