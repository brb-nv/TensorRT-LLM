...
__device__ void forEachScore(float const* __restrict__ rowBase, int64_t blockStride, int32_t rowEnd,
    int32_t initBlocks, int64_t localStart, int numThreads, Func f)
{
    ...
    SmemFinalType& smemFinal)
{
    ...
    for (int round = 0; round < kNumBins / kNumThreadsPerBlock; ++round)
    {
        ...
        if (prefixSum < kTopK)
        {
            int const nextPrefixSum = static_cast<int>(threadIdx.x) == kNumThreadsPerBlock - 1
                ? totalSum
