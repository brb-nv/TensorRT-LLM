...
// Threads-per-block. The kernel is bandwidth-bound (small per-thread work,
// many output stores) so block-size mostly controls occupancy. 256 is a
// safe default for the Hopper / Blackwell targets shared with the MoE
// kernels.
