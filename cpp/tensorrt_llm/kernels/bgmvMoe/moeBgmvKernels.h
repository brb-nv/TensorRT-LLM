...
// Multi-LoRA MoE BGMV shrink: y[slice, pair, rank] += x @ lora_a[expert, lora_id].
//
// T is the activation / LoRA-weight element type (half or __nv_bfloat16).
// wPtr is a device [numSlices, numExperts] int64 table of per-(slice, expert)
// LoRA-A base pointers (reinterpreted to T**); loraStride is the element stride
// between adapters. perPairInput selects the FC2 path where the input row is the
// routed pair (gathered post-activation) instead of the token (FC1 path).
//
...
// Multi-LoRA MoE BGMV expand: project shrink output through LoRA-B.
