...
// ============================================================================
// Additional constants needed for Helix AllToAll
// (These may be added to moeCommKernelsCommon.h in a future refactor)
// ============================================================================

// Size of a 128-byte aligned block (used for bulk async copies)
constexpr int HELIX_BYTES_PER_128B_BLOCK = 128;

// Number of uint64_t elements per 128-byte block
constexpr int HELIX_UINT64_PER_128B_BLOCK = HELIX_BYTES_PER_128B_BLOCK / sizeof(uint64_t);

// Maximum number of groups (warps) per CTA for Helix communication kernels
constexpr int HELIX_MAX_GROUP_COUNT_PER_BLOCK = 8;

/**
 * Ceiling division: compute ceil(a / b) for integers
 */
template <typename T>
inline constexpr T helix_ceil_div(T a, T b)
{
    return (a + b - 1) / b;
}
 ...
 */
template <typename T>
inline constexpr T helix_align_up(T value, T alignment)
{
    return helix_ceil_div(value, alignment) * alignment;
}
...
constexpr int HELIX_FIFO_ENTRY_128B_COUNT = HELIX_FIFO_ENTRY_BYTES / HELIX_BYTES_PER_128B_BLOCK;
...
inline int computeHelixMaxChannelCount(int cpSize, int smCount = 0)
{
    ...
    int blockCountPerChannel = helix_ceil_div(cpSize, HELIX_MAX_GROUP_COUNT_PER_BLOCK);
