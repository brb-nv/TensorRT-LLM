...
namespace kernels {
// Note: WARP_SIZE and other constants are defined in helixAllToAll.hpp
...
mbarrier_try_wait_parity(uint64_t *addr, uint32_t const &phaseParity) {
#if defined(__CUDACC__) && __CUDA_ARCH__ >= 900
...
cp_async_bulk_s2g(void *dstMem, void const *srcMem, int copySize) {
#if defined(__CUDACC__) && __CUDA_ARCH__ >= 900
...
__device__ __forceinline__ void ldgsts(void *dstShm, void const *srcMem,
                                       bool predGuard) {
#if defined(__CUDACC__) && __CUDA_ARCH__ >= 800
  asm volatile("{\n"
               "  .reg .pred p;\n"
               ...
               "}\n" ::"r"((int)predGuard),
               "r"(__as_ptr_smem(dstShm)), "l"(__as_ptr_gmem(srcMem)),
               "n"(COPY_SIZE));
#endif
}
...
template <int N = 0> __device__ __forceinline__ void cp_async_wait_group() {
#if defined(__CUDACC__) && __CUDA_ARCH__ >= 800
...
__device__ __forceinline__ int g2sAllFields(HelixFieldInfo const *fieldInfo,
  ...
  if constexpr (ALLOW_VARIABLE_FIELD1) {
    ...
    ldgsts<8>(reinterpret_cast<uint8_t *>(shmemBase) + totalSize,
              getPtr(fieldInfo[1], dataIndex), laneId == 0);
...
__device__ __forceinline__ uint64_t *
getFifoBasePtr(HelixAllToAllParams const &params,
  ...
  size_t fifoOffset =
      rankInsideMappedMemory * params.maxChannelCount * FIFO_TOTAL_U64;
  fifoOffset += pairInfo.channel * FIFO_TOTAL_U64;

  return mappedMemory + fifoOffset;
}

__device__ __forceinline__ FifoInfo *
getSenderFifoInfo(HelixAllToAllParams const &params,
                  HelixPairInfo const &pairInfo) {
  ...
  auto *mappedMemory = reinterpret_cast<uint8_t *>(
      params.workspace + mappedMemoryRank * params.workspaceStrideInU64);
  size_t fieldOffset = static_cast<size_t>(FIFO_TOTAL_BYTES) * params.cpSize *
                       params.maxChannelCount;
  mappedMemory += fieldOffset;
  mappedMemory +=
      rankInsideMappedMemory * params.maxChannelCount * sizeof(FifoInfo);
...
__device__ __forceinline__ FifoInfo *
getReceiverFifoInfo(HelixAllToAllParams const &params,
                    HelixPairInfo const &pairInfo) {
...
__device__ __forceinline__ void
startWorkspaceS2G(uint64_t *fifoEntry, uint8_t *shmemBase, int send128ByteCount,
                  int fifo128ByteOffset, int laneId) {
  int copyByteCount = send128ByteCount * BYTES_PER_128B_BLOCK;
  if (laneId == 0) {
    cp_async_bulk_s2g(fifoEntry + fifo128ByteOffset * BYTES_PER_128B_BLOCK /
                                      sizeof(uint64_t),
...
__device__ __forceinline__ void
startWorkspaceS2GReg(uint64_t *fifoEntry, uint8_t *sharedMemoryBase,
                     int send128ByteCount, int fifo128ByteOffset, int laneId) {
...
__device__ __forceinline__ uint64_t
startWorkspaceG2S(uint8_t *shmemBase, uint64_t *fifoEntry,
                  int allLoad128ByteCount, int fifo128ByteOffset,
                  int loaded128ByteCount, uint64_t *smemBar, int laneId) {
  int copyByteCount =
  ...
  if (laneId == 0) {
    cp_async_bulk_g2s(shmemBase + loaded128ByteCount * BYTES_PER_128B_BLOCK,
                      fifoEntry + (fifo128ByteOffset + loaded128ByteCount) *
                                      UINT64_PER_128B_BLOCK,
...
public:
  ...
  checkDataReceivedInShm(uint8_t *sharedMemoryBase, uint64_t step,
  ...
  protoPack(uint8_t *sharedMemoryBase, uint64_t step, int countIn128Bytes,
    ...
    for (int idxIn128BytesBase = halfIndex * 15;
      ...
      int tailFlagInnerIndex =
          tailFlagIndexFromFifoEntry % UINT64_PER_128B_BLOCK;
  ...
  static __device__ __forceinline__ void
  protoUnpack(uint8_t *sharedMemoryBase, uint64_t step, int countIn128Bytes,
              int fifoEntry128ByteIndexBase, int loaded128ByteCount,
              int laneId) {
    uint64_t *aligned128BytesShm =
        reinterpret_cast<uint64_t *>(sharedMemoryBase);
    int halfLaneId = laneId % 16;
  ...
  static __device__ __forceinline__ void
  rearm(uint32_t *u32FifoPtr, uint64_t step, int countIn128Bytes,
        int fifoEntry128ByteIndexBase, int laneId) {
  ...
  computeProtoTransfer128ByteAlignedSize(int compact128ByteSizeBeforeProto) {
    ...
    int tail128ByteSize =
        ceil_div(compact128ByteSizeBeforeProto, 15 * 128) * 128;
...
computeTotalUnpackedSize(HelixFieldInfo const *fields) {
  int size = 0;
...
__global__ void helixAllToAllKernel(HelixAllToAllParams params) {
  ...
  if (enableDebug) {
    printf("[DEBUG] Rank %d: %s block starting - peerRank=%d, channel=%d, "
           "entryCount=%d\n",
           params.cpRank, isSender ? "SEND" : "RECV", peerRank, (int)blockIdx.y,
           params.entryCount);
  }
  ...
  if (isSender) {
    ...
    if (enableDebug) {
      printf("[DEBUG] Rank %d SEND: Initial head=%lld, tail=%lld, "
             "packedSize=%d, 128ByteCount=%d\n",
             params.cpRank, (long long)head, (long long)tail, singlePackedSize,
             singlePacked128ByteCount);
    }
    ...
    for (int entryIdx = pairInfo.channel; entryIdx < params.entryCount;
      ...
      if (enableDebug) {
        printf("[DEBUG] Rank %d SEND: Processing entry %d (dataIndex=%d), "
               "head=%lld, tail=%lld. Starting g2s (%d)\n",
               params.cpRank, entryIdx, dataIndex, (long long)head,
               (long long)tail, getFieldSize(params.sendFields[0]));
      }
      ...
      uint64_t *fifoEntry =
          fifoBase + fifoEntryIndex * (FIFO_ENTRY_BYTES / sizeof(uint64_t));

      if (enableDebug) {
        printf("[DEBUG] Rank %d SEND: Loaded %d bytes to shmem, writing to "
               "FIFO entry %d\n",
      ...
      if (enableDebug) {
        printf("[DEBUG] Rank %d SEND: Completed entry %d, new head=%lld\n",
               params.cpRank, entryIdx, (long long)head);
      }
    ...
    if (enableDebug) {
      printf("[DEBUG] Rank %d RECV: Initial tail=%lld, "
             "packedSize=%d, 128ByteCount=%d\n",
             params.cpRank, (long long)tail, singlePackedSize,
             singlePacked128ByteCount);
    }
    ...
    for (int entryIdx = pairInfo.channel; entryIdx < params.entryCount;
      ...
      if (enableDebug) {
        printf("[DEBUG] Rank %d RECV: Processing entry %d (dataIndex=%d), "
               "tail=%lld\n",
      ...
      s2gAllFields<ALLOW_VARIABLE_FIELD1>(params.recvFields, dataIndex, shmem,
                                          laneId);
...
computeChannelAndGroupCount(int cpSize, HelixFieldInfo const *fields) {
  ...
  TLLM_CUDA_CHECK(cudaDeviceGetAttribute(
...
void initializeHelixWorkspace(uint64_t *local_workspace_ptr, int cpSize,
  ...
  size_t fifoSize =
      static_cast<size_t>(FIFO_TOTAL_BYTES) * cpSize * maxChannelCount;
