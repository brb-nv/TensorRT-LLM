...
__global__ void __launch_bounds__(MAX_THREADS) helix_postprocess_kernel(
    T* output, T const* gathered_o, float2 const* gathered_stats, int cp_size, int kv_lora_rank)
{
    ...
    // all warps except first pre-load the gathered_o elements for the current token/head.
...
void helixPostProcess(HelixPostProcParams<T> const& params, cudaStream_t stream)
{
    ...
    auto get_kernel = [](int cp_dim)
    {
        switch (cp_dim)
        {
        case 0: return helix_postprocess_kernel<0, T>;
        case 1: return helix_postprocess_kernel<1, T>;
        default: return helix_postprocess_kernel<2, T>;
        }
    };
