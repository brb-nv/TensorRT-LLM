...
template <int N>
__device__ inline void warpReduceCorrectedSum(float (&correctedVal)[N],
                                              float (&maxVal)[N],
                                              float (&sumVal)[N]) {
...
static constexpr int MAX_KV_LORA_BYTES =
    (MAX_THREADS - WARP_SIZE) * BYTES_O_PER_THREAD;
...
__global__ void __launch_bounds__(MAX_THREADS)
    helix_postprocess_kernel(T *output, T const *gathered_o,
                             float2 const *gathered_stats, int cp_size,
                             int kv_lora_rank) {
  ...
  int const cp_size_aligned =
      ((cp_size + NUM_PRE_LOAD - 1) / NUM_PRE_LOAD) * NUM_PRE_LOAD;
  ...
  if constexpr (CP_DIM == 0) {
    gathered_o_off = gathered_o + tok_idx * num_heads * kv_lora_rank +
                     head_idx * kv_lora_rank;
  } else if constexpr (CP_DIM == 1) {
    gathered_o_off = gathered_o + tok_idx * cp_size * num_heads * kv_lora_rank +
                     head_idx * kv_lora_rank;
  } else {
    gathered_o_off = gathered_o + tok_idx * num_heads * cp_size * kv_lora_rank +
                     head_idx * cp_size * kv_lora_rank;
  }
  ...
  float4 const *gathered_o_16b =
      reinterpret_cast<float4 const *>(gathered_o_off);
  int gathered_16b_stride;
  if constexpr (CP_DIM == 0) {
    gathered_16b_stride =
        (num_tokens * num_heads * kv_lora_rank) / NUM_O_PER_THREAD;
  } else if constexpr (CP_DIM == 1) {
...
#pragma unroll
    for (int cp_val_idx = 0; cp_val_idx < MAX_CP_VAL_PER_THREAD; ++cp_val_idx) {
      ...
      float2 stats = cp_idx < cp_size ? gathered_stats[stats_idx]
...
#pragma unroll
  ...
  for (int cp_idx_base = NUM_PRE_LOAD; cp_idx_base < cp_size_aligned;
       cp_idx_base += NUM_PRE_LOAD) {
#pragma unroll
    for (int cp_idx = 0; cp_idx < NUM_PRE_LOAD; ++cp_idx) {
#pragma unroll
      for (int o_idx = 0; o_idx < NUM_O_PER_THREAD; ++o_idx) {
        final_sum[o_idx] +=
            static_cast<float>(vals[cp_idx][o_idx]) * corr_vals[cp_idx];
      }
    }
#pragma unroll
    for (int cp_idx = 0; cp_idx < NUM_PRE_LOAD; ++cp_idx) {
      *reinterpret_cast<float4 *>(vals[cp_idx]) =
          cp_idx_base + cp_idx < cp_size
              ? gathered_o_16b[(cp_idx_base + cp_idx) * gathered_16b_stride]
              : make_float4(0.F, 0.F, 0.F, 0.F);
      corr_vals[cp_idx] = cp_idx_base + cp_idx < cp_size
                              ? smem_correction[cp_idx_base + cp_idx]
                              : 0.F;
    }
  }
#pragma unroll
...
#pragma unroll
  ...
  auto *output_off =
      output + tok_idx * num_heads * kv_lora_rank + head_idx * kv_lora_rank;
  output_off += (threadIdx.x - WARP_SIZE) * NUM_O_PER_THREAD;
  *reinterpret_cast<float4 *>(output_off) =
      *reinterpret_cast<float4 *>(output_typed);
}

template <typename T>
void helixPostProcess2(HelixPostProcParams<T> const &params,
                       cudaStream_t stream) {
  ...
  TLLM_CHECK_WITH_INFO(params.cp_size <= MAX_CP,
                       "cp_size > fallback max CP size");

  auto get_kernel = [](int cp_dim) {
    switch (cp_dim) {
    case 0:
      return helix_postprocess_kernel<0, T>;
    case 1:
      return helix_postprocess_kernel<1, T>;
    default:
      return helix_postprocess_kernel<2, T>;
    }
  };
  ...
  config.numAttrs = 1;
  config.attrs = attrs;
  TLLM_CUDA_CHECK(cudaLaunchKernelEx(&config, kernel_instance, params.output,
                                     params.gathered_o, params.gathered_stats,
                                     params.cp_size, params.kv_lora_rank));
}

#define INSTANTIATE_POST_PROC(T)                                               \
  template void helixPostProcess2<T>(HelixPostProcParams<T> const &params,     \
                                     cudaStream_t stream);
