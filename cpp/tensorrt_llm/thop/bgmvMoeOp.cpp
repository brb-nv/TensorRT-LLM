...
// y[num_slices, num_pairs, rank] += x @ lora_a[expert, lora_id]. y is accumulated
// in place (caller zero-inits). x is [num_tokens, feat_in] (FC1) or
// [num_pairs, feat_in] (FC2, per_pair_input=true).
void bgmvMoeShrinkOp(at::Tensor& y, at::Tensor const& x, at::Tensor const& wPtr, at::Tensor const& sortedTokenIds,
    at::Tensor const& expertIds, at::Tensor const& loraIndices, int64_t loraStride, bool perPairInput)
{
    ...
    TORCH_CHECK(wPtr.dim() == 2, "bgmv_moe_shrink: w_ptr must be 2D [num_slices, num_experts].");
    ...
    int64_t const numExperts = wPtr.size(1);
    ...
    case at::ScalarType::Half:
        ok = tensorrt_llm::kernels::bgmv_moe::bgmvMoeShrink<half>(static_cast<half*>(y.data_ptr()),
            ...
            static_cast<int64_t const*>(loraIndices.data_ptr()), featIn, featOut, numPairs, numSlices, numExperts,
            numTokens, loraStride, perPairInput, stream);
        break;
    case at::ScalarType::BFloat16:
        ok = tensorrt_llm::kernels::bgmv_moe::bgmvMoeShrink<__nv_bfloat16>(
            ...
            featIn, featOut, numPairs, numSlices, numExperts, numTokens, loraStride, perPairInput, stream);
        break;
...
void bgmvMoeExpandOp(at::Tensor& y, at::Tensor const& x, at::Tensor const& wPtr, at::Tensor const& sortedTokenIds,
    ...
    at::Tensor const& sliceStartLoc, int64_t firstFeatOut, int64_t loraStride, bool finalize)
{
    CHECK_INPUT(sortedTokenIds, at::ScalarType::Long);
    CHECK_INPUT(expertIds, at::ScalarType::Long);
    CHECK_INPUT(loraIndices, at::ScalarType::Long);
    ...
    TORCH_CHECK(wPtr.dim() == 2, "bgmv_moe_expand: w_ptr must be 2D [num_slices, num_experts].");
    ...
    case at::ScalarType::Half:
        ok = tensorrt_llm::kernels::bgmv_moe::bgmvMoeExpand<half>(static_cast<float*>(y.data_ptr()),
            ...
            numExperts, totalFeatOut, numTokens, loraStride, finalize, stream);
        break;
    case at::ScalarType::BFloat16:
...
TORCH_LIBRARY_FRAGMENT(trtllm, m)
{
    m.def(
        ...
        "Tensor expert_ids, Tensor lora_indices, int lora_stride, bool per_pair_input) -> ()");
    m.def(
        ...
        "int first_feat_out, int lora_stride, bool finalize) -> ()");
}
