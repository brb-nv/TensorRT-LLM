...
inline torch::Tensor helix_post_process_native_impl_cp_dim2(
...
template <typename T, typename Fn>
inline torch::Tensor helix_post_process_native_impl_cp_dim1(
    torch::Tensor const& gathered_o, torch::Tensor const& gathered_stats, double scale, Fn fn)
{
    CHECK_TH_CUDA(gathered_o);
    CHECK_CONTIGUOUS(gathered_o);
    ...
    torch::Tensor const& gathered_o, torch::Tensor const& gathered_stats, double scale, int64_t cp_dim)
{
    ...
    if (cp_dim == 1)
    {
        ...
        if (gathered_o.scalar_type() == at::ScalarType::Half)
        {
            return helix_post_process_native_impl_cp_dim1<__half>(
                gathered_o, gathered_stats, scale, tensorrt_llm::kernels::helixPostProcessNativeV2<__half>);
        }
        else if (gathered_o.scalar_type() == at::ScalarType::BFloat16)
        {
#ifdef ENABLE_BF16
            return helix_post_process_native_impl_cp_dim1<__nv_bfloat16>(
    ...
    else
    {
        ...
        if (gathered_o.scalar_type() == at::ScalarType::Half)
        {
            return helix_post_process_native_impl_cp_dim2<__half>(
                gathered_o, gathered_stats, scale, tensorrt_llm::kernels::helixPostProcessNative<__half>);
        }
        else if (gathered_o.scalar_type() == at::ScalarType::BFloat16)
        {
#ifdef ENABLE_BF16
            return helix_post_process_native_impl_cp_dim2<__nv_bfloat16>(
                gathered_o, gathered_stats, scale, tensorrt_llm::kernels::helixPostProcessNative<__nv_bfloat16>);
#else
            TLLM_THROW("BFloat16 must be enabled to use helix_post_process_native with bf16 tensors.");
#endif
        }
