...
inline torch::Tensor
helix_post_process_impl(torch::Tensor const &gathered_o,
                        torch::Tensor const &gathered_stats, double scale,
                        int cp_dim, Fn fn) {
  ...
  TORCH_CHECK(gathered_o.dim() == 4, "gathered_o must be 4D tensor [cp_size, "
                                     "num_tokens, num_heads, kv_lora_rank] "
                                     "where cp_size is at index cp_dim");
  TORCH_CHECK(
      gathered_stats.dim() == 4,
  ...
  TORCH_CHECK(gathered_o.sizes()[cp_dim] == cp_size,
              "gathered_o cp_dim must match cp_size");
  TORCH_CHECK(gathered_o.sizes()[tokens_dim] == num_tokens,
              "gathered_o tokens_dim must match num_tokens");
  TORCH_CHECK(gathered_o.sizes()[heads_dim] == num_heads,
              "gathered_o heads_dim must match num_heads");

  TORCH_CHECK(gathered_stats.sizes()[3] == 2,
              "gathered_stats last dimension must be 2");

  // Check data types
  TORCH_CHECK(gathered_o.scalar_type() == at::ScalarType::Half ||
                  gathered_o.scalar_type() == at::ScalarType::BFloat16,
              "gathered_o must be half or bfloat16");
  TORCH_CHECK(gathered_stats.scalar_type() == at::ScalarType::Float,
              "gathered_stats must be float32");

  // Check alignment requirements for gathered_o (16-byte aligned for async
  // memcpy)
  TORCH_CHECK(reinterpret_cast<uintptr_t>(gathered_o.data_ptr()) % 16 == 0,
              "gathered_o must be 16-byte aligned");
  ...
  TORCH_CHECK((kv_lora_rank * data_type_size) % 16 == 0,
              "kv_lora_rank * sizeof(data_type) must be a multiple of 16");
...
inline torch::Tensor
helix_post_process_impl_version(torch::Tensor const &gathered_o,
                                torch::Tensor const &gathered_stats,
                                double scale, int64_t cp_dim) {
  if constexpr (version == 1) {
    ...
    if (gathered_o.scalar_type() == at::ScalarType::Half) {
      return helix_post_process_impl<__half>(
          gathered_o, gathered_stats, scale, int(cp_dim),
          tensorrt_llm::kernels::helixPostProcess<__half>);
    ...
    TORCH_CHECK(cp_dim >= 0 && cp_dim <= 2,
                "cp_dim must be 0, 1, or 2 for version 2");
    if (gathered_o.scalar_type() == at::ScalarType::Half) {
      return helix_post_process_impl<__half>(
          gathered_o, gathered_stats, scale, int(cp_dim),
          tensorrt_llm::kernels::helixPostProcess2<__half>);
    } else if (gathered_o.scalar_type() == at::ScalarType::BFloat16) {
      return helix_post_process_impl<__nv_bfloat16>(
          gathered_o, gathered_stats, scale, int(cp_dim),
      ...
      TLLM_THROW(
          "helix_post_process2 only supports half and bfloat16 tensors.");
    }
  } else {
...
TORCH_LIBRARY_FRAGMENT(helix, m) {
  m.def("helixPostProcess(Tensor gathered_o, Tensor gathered_stats, float "
        "scale, int cp_dim) -> Tensor");
  m.def("helixPostProcess2(Tensor gathered_o, Tensor gathered_stats, float "
        "scale, int cp_dim) -> Tensor");
}
