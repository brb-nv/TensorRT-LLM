...
// ============================================================================
// Native Helix All-to-All Operations (from helixAllToAllOp.cpp)
// ============================================================================

#include "tensorrt_llm/kernels/helixAllToAll.hpp"
#include "tensorrt_llm/common/cudaUtils.h"
#include "tensorrt_llm/thop/thUtils.h"

/**
 * Helix All-to-All Native operation with two fields.
 *
...
helixAllToAllNative(torch::Tensor field0, torch::Tensor field1,
  ...
  // Type checks
  TORCH_CHECK(field0.scalar_type() == at::ScalarType::Half ||
                  field0.scalar_type() == at::ScalarType::BFloat16,
  ...
  TORCH_CHECK(field0.dim() == field1.dim(),
              "field0 and field1 must have same number of dimensions");
  ...
  TORCH_CHECK(field1.size(-1) % 2 == 0 && field1.size(-1) >= 2,
              "field1 last dimension must be divisible by 2 (float2)");
  bool allowVariableField1 = field1.size(-1) > 2;

  // Check that leading dimensions match
  for (int i = 0; i < field0.dim() - 2; i++) {
    TORCH_CHECK(
        field0.size(i) == field1.size(i),
        "field0 and field1 must have matching dimensions except last two");
  }
  TORCH_CHECK(field0.size(-1) * field0.element_size() % 16 == 0,
              "field0 must be aligned to 16 bytes");
  ...
  torch::Tensor field0_3d =
      field0.reshape({entry_count, cp_size, kv_lora_rank});
  torch::Tensor field1_3d =
      field1.reshape({entry_count, cp_size, field1.size(-1)});
  ...
  torch::Tensor field0_out_3d =
      field0_out.reshape({entry_count, cp_size, kv_lora_rank});
  torch::Tensor field1_out_3d =
      field1_out.reshape({entry_count, cp_size, field1.size(-1)});
  ...
  params.sendFields[0].dataPtr =
      reinterpret_cast<uint8_t *>(field0_3d.data_ptr());
  ...
  params.recvFields[0].dataPtr =
      reinterpret_cast<uint8_t *>(field0_out_3d.data_ptr());
  ...
  params.sendFields[1].dataPtr =
      reinterpret_cast<uint8_t *>(field1_3d.data_ptr<float>());
  ...
  params.recvFields[1].dataPtr =
      reinterpret_cast<uint8_t *>(field1_out_3d.data_ptr<float>());
  ...
  params.maxChannelCount =
      tensorrt_llm::kernels::computeHelixMaxChannelCount(cp_size);

  // Launch kernel
  auto stream = at::cuda::getCurrentCUDAStream();
  tensorrt_llm::kernels::launchHelixAllToAll(params, allowVariableField1,
                                             stream);
 ...
 */
int64_t getHelixWorkspaceSizePerRankNative(torch::Tensor __dummy__, int64_t cp_size) {
 ...
 */
void initializeHelixWorkspaceNative(torch::Tensor workspace, int64_t cp_rank,
                              int64_t cp_size) {
  CHECK_TH_CUDA(workspace);
  CHECK_TYPE(workspace, at::ScalarType::UInt64);
  ...
  TORCH_CHECK(local_workspace_ptr ==
              ...
              "workspace");
  tensorrt_llm::kernels::initializeHelixWorkspace(local_workspace_ptr, cp_size,
                                                  stream);
}

/**
 * Helix All-to-All Fused Native operation with two fields.
 *
 * Input tensors have shape [..., cp_size, kv_lora_rank] for field0 and [...,
 * cp_size, 2] for field1. The operation exchanges data along the cp_size
 * dimension across all ranks.
 ...
 * @return tuple of (field0_out, field1_out) with same shapes as inputs
 */
torch::Tensor helixFusedAllToAllNative(torch::Tensor field0, torch::Tensor field1,
                                 torch::Tensor workspace, int64_t cp_rank,
                                 int64_t cp_size) {
  ...
  // Shape validation
  TORCH_CHECK(field0.dim() == 4, "field0 must have 4 dimensions");
  TORCH_CHECK(field1.dim() == 4, "field1 must have 4 dimensions");

  // Get dimensions
  int kv_lora_rank = field0.size(3);
  TORCH_CHECK(field0.size(1) == cp_size && field1.size(1) == cp_size,
              "field0/1 second dimension must equal cp_size");
  TORCH_CHECK(field1.size(3) == 2, "field1 last dimension must be 2 (float2)");

  // Check that leading dimensions match
  for (int i = 0; i < field0.dim() - 1; i++) {
    TORCH_CHECK(field0.size(i) == field1.size(i),
                "field0 and field1 must have matching dimensions except last");
  }
  TORCH_CHECK(kv_lora_rank * field0.element_size() % 16 == 0,
              "field0 must be aligned to 16 bytes");
  ...
  // Allocate output tensors (same shape as input)
  auto out_sizes = field0.sizes().vec();
  // remove the cp_size dimension
  out_sizes.erase(std::next(out_sizes.begin()));
  torch::Tensor field_out = torch::empty(out_sizes, field0.options());

  // Setup parameters
  tensorrt_llm::kernels::fused::HelixFusedAllToAllParams params;

  // Field 0 (variable size half)
  params.sendFields[0].dataPtr = reinterpret_cast<uint8_t *>(field0.data_ptr());
  params.sendFields[0].numTokens = field0.size(0);
  params.sendFields[0].numHeads = field0.size(2);
  params.sendFields[0].headDim = kv_lora_rank;
  params.sendFields[0].elementSize = field0.element_size();

  // Field 1 (single float2)
  params.sendFields[1].dataPtr =
      reinterpret_cast<uint8_t *>(field1.data_ptr<float>());
  params.sendFields[1].numTokens = field1.size(0);
  params.sendFields[1].numHeads = field1.size(2);
  params.sendFields[1].headDim = 2;
  params.sendFields[1].elementSize = field1.element_size();

  params.recvField.dataPtr = reinterpret_cast<uint8_t *>(field_out.data_ptr());
  params.recvField.numTokens = field_out.size(0);
  params.recvField.numHeads = field_out.size(1);
  params.recvField.headDim = kv_lora_rank;
  params.recvField.elementSize = field_out.element_size();

  if (field0.scalar_type() == at::ScalarType::Half) {
    params.postProcDataType = tensorrt_llm::kernels::fused::DataType::FP16;
  } else if (field0.scalar_type() == at::ScalarType::BFloat16) {
    params.postProcDataType = tensorrt_llm::kernels::fused::DataType::BF16;
  }
  ...
  params.channelCount = 0; // auto-compute
  params.maxChannelCount =
      tensorrt_llm::kernels::fused::computeHelixFusedMaxChannelCount(cp_size);

  // Launch kernel
  auto stream = at::cuda::getCurrentCUDAStream();
  tensorrt_llm::kernels::fused::launchHelixFusedAllToAll(params, stream);

  return field_out;
}

/**
 * Get workspace size per rank in bytes for fused version.
 * Use dummy tensor argument to allow using torch.ops
 */
int64_t getHelixFusedWorkspaceSizePerRankNative(torch::Tensor __dummy__,
                                          int64_t cp_size) {
  return tensorrt_llm::kernels::fused::computeHelixFusedWorkspaceSizePerRank(
      cp_size);
}

/**
 * Initialize workspace for helix fused all-to-all
 */
void initializeHelixFusedWorkspaceNative(torch::Tensor workspace, int64_t cp_rank,
                                   int64_t cp_size) {
  CHECK_TH_CUDA(workspace);
  CHECK_TYPE(workspace, at::ScalarType::UInt64);
  TORCH_CHECK(workspace.dim() == 2, "workspace must be 2D");
  ...
  TORCH_CHECK(local_workspace_ptr ==
              ...
              "local_workspace_ptr must be at the correct offset in the global "
              "workspace");
  tensorrt_llm::kernels::fused::initializeHelixFusedWorkspace(
      local_workspace_ptr, cp_size, stream);
}

// Update TORCH_LIBRARY_FRAGMENT to include native versions
TORCH_LIBRARY_FRAGMENT(trtllm, m_native) {
  m_native.def("helixAllToAllNative(Tensor field0, Tensor field1, Tensor workspace, int "
        "cp_rank, int cp_size) -> (Tensor, Tensor)");
  m_native.def("getHelixWorkspaceSizePerRankNative(Tensor __dummy__, int cp_size) -> int");
  m_native.def("initializeHelixWorkspaceNative(Tensor workspace, int cp_rank, int cp_size) "
        "-> ()");
  m_native.def(
      "helixFusedAllToAllNative(Tensor field0, Tensor field1, Tensor workspace, int "
      "cp_rank, int cp_size) -> Tensor");
  m_native.def("getHelixFusedWorkspaceSizePerRankNative(Tensor __dummy__, int cp_size) -> "
        "int");
  m_native.def("initializeHelixFusedWorkspaceNative(Tensor workspace, int cp_rank, int "
        "cp_size) "
        "-> ()");
}

TORCH_LIBRARY_IMPL(trtllm, CUDA, m_native_cuda) {
  m_native_cuda.impl("helixAllToAllNative", &helixAllToAllNative);
  m_native_cuda.impl("getHelixWorkspaceSizePerRankNative", &getHelixWorkspaceSizePerRankNative);
  m_native_cuda.impl("initializeHelixWorkspaceNative", &initializeHelixWorkspaceNative);
  m_native_cuda.impl("helixFusedAllToAllNative", &helixFusedAllToAllNative);
  m_native_cuda.impl("getHelixFusedWorkspaceSizePerRankNative",
         &getHelixFusedWorkspaceSizePerRankNative);
  m_native_cuda.impl("initializeHelixFusedWorkspaceNative", &initializeHelixFusedWorkspaceNative);
}
