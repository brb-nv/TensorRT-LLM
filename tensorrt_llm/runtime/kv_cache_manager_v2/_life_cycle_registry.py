...
class AttnLifeCycle(NamedTuple):
    ...
    # Opaque grouping tag: participates in life-cycle identity (equality / hash)
    # but is intentionally ignored by ``get_stale_range``. Two layers with the
    # same window / sink but different ``group_tag`` become distinct life cycles
    # (distinct pool groups + page chains) while retaining identical staleness
    # behavior. Default ``0`` preserves historical single-group behavior.
    group_tag: int = 0
    ...
    def make(
        ...
        return AttnLifeCycle(window_size, num_sink_blocks, group_tag)
