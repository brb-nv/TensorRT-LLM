...
class MiniMaxM3MsaSparseAttentionMetadata(TrtllmAttentionMetadata):
    ...
    # Eager (prefill / mixed) plans. Built once per step in prepare() and
    # reused by every layer, so a prefill / chunked-prefill forward builds the
    # three layer-invariant plans once instead of rebuilding proxy + GQA inline
    # in each of the 57 sparse layers (and dense inline in layers 0-2). These
    # are plain plan tuples (no graph-stable buffers): prefill runs eagerly and
    # is never CUDA-graph captured.
    ...
    def _msa_main_kv_is_fp8(self) -> bool:
        ...
        The eager GQA / dense plans must pass ``use_fp8_kvcache`` so the inline
        sparse-prefill path selects the FP8 AOT kernels (it is a no-op for the
        decode planner). Mirrors the ``k_paged.dtype`` check in
        ``run_msa_paged_gqa``.
        """
    ...
    def _build_step_plans(self) -> None:
        ...
        Runs in prepare(), outside CUDA graph capture. The proxy / GQA / dense
        plans depend only on the per-step sparse geometry (qo/kv lengths, head
        counts, topk, page size), never on the layer, so they are built once
        here and reused by every layer:

          * Pure-decode batches mirror the plans into the CUDA-graph-stable
            ``_MsaGraphSafePlan`` buffers (surfaced by ``msa_decode_*_plan``),
            because decode is captured and the plan worklists must keep a fixed
            address across replays.
          * Prefill / chunked-prefill / mixed batches run eagerly (never
            captured), so the plans are stored as plain tuples
            (``msa_eager_*_plan``). This replaces the previous behaviour of
            rebuilding the proxy + GQA plans inline in every sparse layer (57x)
            and the dense plan inline in layers 0-2 (3x) — ~117 identical plan
            builds per prefill step collapse to 3.
        """
        ...
        # The main-attention GQA / dense plans need use_fp8_kvcache so the
        ...
        if not is_decode:
            # Prefill / mixed: run eagerly, so keep the plain plan tuples and
            # leave the graph-safe owners reset. The indexer computes its own
            # per-query valid-block count on the eager path.
...
class MiniMaxM3MsaSparseAttention(TrtllmAttention):
    ...
    def run_indexer(
        ...
        *,
        ...
        # mixed passes the prebuilt eager proxy plan (built once per step in
        # prepare) and lets the kernel allocate the score buffer and the
        # indexer compute the per-query valid-block count. Only when neither is
        # present (e.g. a standalone test that skips prepare) does select_blocks
        # build the proxy plan inline.
