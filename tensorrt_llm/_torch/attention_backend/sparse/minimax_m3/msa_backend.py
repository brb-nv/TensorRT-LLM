...
class MiniMaxM3MsaSparseAttention(TrtllmAttention):
    ...
    def run_indexer(
        ...
        *,
        ...
        # idx_q and idx_k arrive as strided column views of the fused projection
        # output, so reshape rather than view: it yields the same zero-copy view
        # whenever the strides allow one instead of refusing outright. The scorer
        # and cache writer below both honor their source strides.
        ...
        if configured_for_fp8:
            ...
            # The fused per-layer write (msa_write_layer_caches) already stored
            # this layer's index-K, so writing it again here would be redundant.
