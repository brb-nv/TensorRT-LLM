...
class MiniMaxM3MsaSparseAttentionMetadata(TrtllmAttentionMetadata):
    ...
    # reused by every sparse layer's indexer. This keeps the indexer free of a
    # per-layer pageable host-to-device copy, which blocks the host with a
    # cudaStreamSynchronize. _msa_eager_all_blocks_empty caches the host-side
    # empty-selection result so the indexer can short-circuit without a device
    # read. _msa_eager_n_valid_buf is the persistent backing store for the view.
