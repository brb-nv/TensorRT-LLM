        ...
        for request in generation_requests:
            ...
            for beam in range(beam_width):
                ...
                if self.mapping.has_cp_helix():
                    ...
                    helix_is_inactive_rank_all_ranks = torch.tensor(
                        helix_is_inactive_rank_all_ranks, dtype=torch.long)
                    assert torch.sum(
                        helix_is_inactive_rank_all_ranks
                    ) == self.mapping.cp_size - 1, "There should be only one active rank in helix parallelism."
                    #####################################################################################
