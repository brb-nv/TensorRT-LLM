...
# Dense-layer SDPA (layers 0-2) runs with non-contiguous transposed Q/K/V
# views and a broadcast boolean attn_mask. On Blackwell (SM100+) PyTorch's
# SDPA auto-dispatch prefers the cuDNN fused-attention backend, whose graph
# fails at runtime for this combination (``mha_graph.execute(...).is_good()``
# returns false). Restrict SDPA to the memory-efficient and math backends,
# which reliably support arbitrary bool masks and non-contiguous inputs. The
# flash backend is intentionally excluded because it does not accept a
# dense attn_mask.
