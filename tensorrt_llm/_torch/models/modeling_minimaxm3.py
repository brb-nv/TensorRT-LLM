...
class MiniMaxM3DecoderLayer(DecoderLayer):
        ...
        with nvtx_range_debug(f"layer{self.layer_idx}.{ffn_kind}"):
            if self.block_sparse_moe is not None:
                ...
                hidden_states, residual = self.forward_mlp(
                    hidden_states, residual, spec_metadata
                )
    ...
    def _capture_disables_post_fusion(self, spec_metadata: Optional[SpecMetadata]) -> bool:
        """Whether Eagle3 captures this layer's feed-forward output.

        Capture wants the fully TP-reduced hidden states from *before* the layer
        boundary norm, so POST fusion (which folds that norm into the
        feed-forward AllReduce) has to stay off on a capture layer. Mirrors how
        DeepSeek-V3 clears POST_MOE_FUSION / POST_MLP_FUSION for capture layers.
        """
        if spec_metadata is None or not spec_metadata.is_layer_capture(self.layer_idx):
            return False
    ...
    def forward_MoE(
        ...
        capture = self._capture_disables_post_fusion(spec_metadata)
