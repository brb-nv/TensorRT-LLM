    ...
    def _dense_forward(
        ...
        **kwargs,
        ...
        with nvtx_range_debug(f"layer{self.layer_idx}.dense_attn.o_proj"):
            # ``all_reduce_params`` lets the decoder defer the o_proj output
            # AllReduce so it can be fused with post_attention_layernorm
            # (RESIDUAL_RMS_NORM). Passing None preserves the standalone o_proj
            # reduction (single-GPU / attention-DP / fusion-disabled paths).
...
class MiniMaxM3DecoderLayer(DecoderLayer):
    ...
    def __init__(
        ...
        # Layer-boundary RMSNorms are constructed as plain (non-Gemma) norms so
        # they can drive the fused AllReduce+residual+RMSNorm epilogue
        # (AllReduceFusionOp.RESIDUAL_RMS_NORM), whose kernel applies a plain
        # ``weight * x`` scaling with no Gemma ``(1 + weight)`` offset. When the
        # checkpoint stores Gemma norms (``use_gemma_norm=True``), the loader
        # folds ``(1 + weight)`` into the stored weight at load time (see
        # :func:`_fold_gemma_boundary_norm_weights`), so the runtime norm is
        # numerically identical to the original Gemma norm on every path
        # (single-GPU, attention-DP, and the fused TP path). The per-head
        ...
        #     (PRE fusion), and
        #   * the MoE/MLP output AllReduce into the next layer's input_layernorm
        #     (POST fusion, wired via ``next_layer_layernorm`` in setup_aliases).
        # Fusion is only meaningful when there is a real cross-rank reduction to
        # fold, i.e. TP>1 and not attention-DP (each DP rank owns independent
        # tokens, so no attention/MoE AllReduce happens there). An env override
        # matches the DeepSeek-V3 escape hatch.
        self.enable_fusion = os.environ.get(
            "TRTLLM_MINIMAX_M3_EAGER_FUSION_DISABLED", "0"
        ) == "0"
        ...
        # Wired by ``MiniMaxM3ForCausalLM.setup_aliases`` after weight load to
        # the next layer's ``input_layernorm`` (or the final model ``norm`` for
        # the last layer). ``None`` disables POST fusion / boundary-norm folding.
    ...
    def forward(
        ...
        **kwargs,
        ...
        # previous layer as its ``next_layer_layernorm``, so ``residual`` is not
        # None here and this block is skipped (matches DeepSeek-V3).
        if residual is None:
        ...
        # single-GPU / attention-DP / fusion-disabled behaviour exactly).
    ...
    def _apply_pre_feed_forward_norm(
        ...
        add+RMSNorm (the attention already reduced its own output)."""
        if self.pre_feed_forward_fusion:
    ...
    def _apply_next_layer_layernorm(
        ...
        fused with ``next_layer_layernorm`` (the next layer's input_layernorm,
        or the final model norm for the last layer). Off the fusion path it is
        the plain add+RMSNorm. When ``next_layer_layernorm`` has not been wired
        (e.g. a standalone unit test that never ran ``setup_aliases``) the
    ...
    def _feed_forward_all_reduce_params(self) -> Optional[AllReduceParams]:
        """AllReduce params handed to the MoE / dense-MLP output projection.

        Disables the module's internal output AllReduce when POST fusion will
        fold it into ``next_layer_layernorm``; otherwise ``None`` preserves the
        module's own reduction (single-GPU / attention-DP / fusion-disabled)."""
        if self.post_feed_forward_fusion:
...
class MiniMaxM3Model(DecoderModel):
    ...
    def __init__(self, model_config: "ModelConfig[PretrainedConfig]"):
        ...
        # as the last layer's ``next_layer_layernorm``, so the last MoE/MLP
        # output AllReduce folds into it via RESIDUAL_RMS_NORM. The Gemma
        # ``(1 + weight)`` offset is folded into the stored weight at load time
        # (see :func:`_fold_gemma_boundary_norm_weights`).
        self.norm = RMSNorm(
            hidden_size=config.hidden_size,
            eps=config.rms_norm_eps,
            dtype=config.torch_dtype,
    ...
    def forward(
        ...
        **kwargs,
        ...
        # When ``setup_aliases`` has chained the final norm into the last
        # decoder layer (``next_layer_layernorm = self.norm``), the last layer's
        # boundary step already applied it (fused or plain), so ``hidden_states``
        # is normed and this is skipped. The fallback covers paths that never
        # ran ``setup_aliases`` (e.g. standalone unit tests): there the last
        # layer returns the unnormed (hidden_states, residual) pair and the
        # final add+RMSNorm is applied here.
...
# Layer-boundary RMSNorms whose Gemma ``(1 + weight)`` scaling is folded into
# the stored weight at load time so the runtime norm is a plain RMSNorm (see
...
def _fold_gemma_boundary_norm_weights(weights):
    ...
    MiniMax-M3 stores every RMSNorm as a Gemma norm (``use_gemma_norm=True``),
    which computes ``(1 + weight) * x``. The layer-boundary norms are
    constructed as plain norms (``use_gemma=False``, ``weight * x``) so they can
    drive the fused AllReduce+RMSNorm kernels, so their stored weights must be
    pre-incremented by 1.0. This is a numerically exact, load-time-only rewrite;
    the resulting norm is identical to the original Gemma norm on every path.

    Only the decoder ``input_layernorm`` / ``post_attention_layernorm`` and the
    final ``model.norm`` are touched. A no-op for keys that are absent (partial
    load) so it is safe to call unconditionally, but it must only run when the
    checkpoint actually uses Gemma norms (guarded by the caller).
    """
...
class MiniMaxM3ForCausalLM(DecoderModelForCausalLM[MiniMaxM3Model, PretrainedConfig]):
    ...
    def setup_aliases(self) -> None:
        ...
        layer's ``input_layernorm``; the last layer chains the final model
        ``norm`` so its output AllReduce folds the final normalization too.
        """
