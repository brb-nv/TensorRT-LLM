...
from ...utils import (add_start_docstrings,
                      add_start_docstrings_to_model_forward, can_return_tuple,
...
class Gemma3TextScaledWordEmbedding(nn.Embedding):
    ...
    def forward(self, input_ids: torch.Tensor):
        ...
        return input_embeddings * self.embed_scale.to(self.weight.dtype)
...
class Gemma3Attention(nn.Module):
    ...
    def __init__(self, config: Gemma3TextConfig, layer_idx: int):
        ...
        self.q_norm = Gemma3RMSNorm(dim=config.head_dim,
                                    eps=config.rms_norm_eps)
        self.k_norm = Gemma3RMSNorm(dim=config.head_dim,
                                    eps=config.rms_norm_eps)
...
class Gemma3DecoderLayer(nn.Module):
    ...
    def __init__(self, config: Gemma3TextConfig, layer_idx: int):
        ...
        self.input_layernorm = Gemma3RMSNorm(self.hidden_size,
                                             eps=config.rms_norm_eps)
        self.post_attention_layernorm = Gemma3RMSNorm(self.hidden_size,
                                                      eps=config.rms_norm_eps)
        self.pre_feedforward_layernorm = Gemma3RMSNorm(self.hidden_size,
                                                       eps=config.rms_norm_eps)
        self.post_feedforward_layernorm = Gemma3RMSNorm(self.hidden_size,
                                                        eps=config.rms_norm_eps)
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.is_sliding and attention_mask is not None:  # efficient SDPA and no padding
            ...
            effective_seq_len = max(cache_position.shape[0],
                                    self.sliding_window)
            ...
            # from the left, with an offset if we are beyond the sliding window
                ...
                sliding_window_mask = torch.tril(torch.ones_like(
                    attention_mask, dtype=torch.bool),
                                                 diagonal=-self.sliding_window)
                attention_mask = torch.where(sliding_window_mask, min_dtype,
                                             attention_mask)
                ...
                offset = max(0, offset)
                attention_mask = attention_mask[:, :, :, offset:offset +
                                                effective_seq_len]
        ...
        if print_condition:
            save_tensor_to_npy(hidden_states,
                               f"attention_input_{self.layer_idx}")
        ...
        if print_condition:
            save_tensor_to_npy(hidden_states,
                               f"attention_output_{self.layer_idx}")
        ...
        if print_condition:
            save_tensor_to_npy(hidden_states, f"mlp_input_{self.layer_idx}")
        hidden_states = self.mlp(hidden_states)
        if print_condition:
            save_tensor_to_npy(hidden_states, f"mlp_output_{self.layer_idx}")
        hidden_states = self.post_feedforward_layernorm(hidden_states)
        if print_condition:
            save_tensor_to_npy(hidden_states,
                               f"mlp_output_post_layernorm_{self.layer_idx}")
        hidden_states = residual + hidden_states
        if print_condition:
            save_tensor_to_npy(hidden_states,
                               f"mlp_output_with_residual_{self.layer_idx}")
...
class Gemma3TextModel(Gemma3PreTrainedModel):
        ...
        **flash_attn_kwargs: Unpack[FlashAttentionKwargs],
        ...
        if last_cache_position is None:
            ...
            if attention_mask is not None:
                ...
                last_cache_position = (attention_mask.shape[-1]
                                       if attention_mask.dim() == 2 else
        ...
        position_embeddings_global = self.rotary_emb(hidden_states,
                                                     position_ids)
    ...
    def _update_causal_mask(
        ...
        if isinstance(past_key_values, (HybridCache, StaticCache)):
            ...
            target_length = attention_mask.shape[
                -1] if attention_mask is not None else input_tensor.shape[1]
...
class Gemma3ForCausalLM(Gemma3PreTrainedModel, GenerationMixin):
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        # This is needed to correctly slice the mask without data-dependent slicing later on if using dynamo tracing
        # (retrieving the same value from `cache_position` later on would crash dynamo)
        model_inputs["last_cache_position"] = attention_mask.shape[
            -1] if attention_mask is not None else 0
        if logits_to_keep is None:
            _ = model_inputs.pop("logits_to_keep", None)

        if (isinstance(past_key_values, HybridCache)
                and attention_mask.ndim == 2 and
                not self.config._attn_implementation == "flash_attention_2"):
...
class Gemma3ForConditionalGeneration(Gemma3PreTrainedModel, GenerationMixin):
    ...
    def _update_causal_mask(
        ...
        if using_static_cache:
            ...
            target_length = (attention_mask.shape[-1] if isinstance(
                attention_mask, torch.Tensor) else cache_position[0] +
                             sequence_length + 1)
        ...
        causal_mask = causal_mask[None,
                                  None, :, :].expand(inputs_lead_dim, 1, -1, -1)
    ...
    @deprecate_kwarg("num_logits_to_keep",
                     ...
                     new_name="logits_to_keep")
    @add_start_docstrings_to_model_forward(GEMMA3_INPUTS_DOCSTRING)
    @replace_return_docstrings(output_type=Gemma3CausalLMOutputWithPast,
                               config_class=_CONFIG_FOR_DOC)
    def forward(
        self,
        input_ids: Optional[torch.LongTensor] = None,
        ...
        **lm_kwargs,
        ...
        if pixel_values is not None:
            ...
            if not is_torchdynamo_compiling() and inputs_embeds[
                ...
                image_tokens_in_text = (special_image_mask).sum(dim=1).sum(
                    dim=0)[0]
                raise ValueError(
        ...
        if labels is not None and self.pad_token_id in labels:
            ...
            labels = torch.where(input_ids == self.pad_token_id,
                                 self.config.ignore_index, labels)

        causal_mask = self._update_causal_mask(attention_mask, token_type_ids,
                                               past_key_values, cache_position,
                                               inputs_embeds, is_training)
        ...
        if labels is not None:
            ...
            if attention_mask is not None:
                ...
                shift_attention_mask = attention_mask[:, -shift_logits.
                                                      shape[1]:].to(
                                                          logits.device)
                shift_logits = shift_logits[shift_attention_mask.to(
                    logits.device) != 0].contiguous()
                shift_labels = shift_labels[shift_attention_mask.to(
                    shift_labels.device) != 0].contiguous()
            else:
...
__all__ = [
    "Gemma3PreTrainedModel", "Gemma3TextModel", "Gemma3ForCausalLM",
